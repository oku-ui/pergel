...
<template>
  <details :open="open" @toggle="onToggle">
    <summary class="hover:bg-active p4 cursor-pointer select-none" :class="collapse ? '' : 'pointer-events-none'">
      <NIconTitle :icon="icon" :text="text" text-xl transition :class="[open ? 'op100' : 'op60', headerClass]">
        ...
        <slot name="actions" />
        <NIcon
          v-if="collapse" icon="carbon-chevron-down" class="chevron" cursor-pointer place-self-start text-base
          op75 transition duration-500
        />
      </NIconTitle>
    </summary>
    <div
      v-show="open" class="flex-gap2 pb6 pt2 flex flex-col"
